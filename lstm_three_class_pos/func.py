...
class ptr_net:
    ...
    def __call__(self, init, match, d, mask):
        ...
        with tf.variable_scope(self.scope):
            ...
            # # all_predict=tf.square(logits2-logits1)
            # all_predict=tf.nn.softmax(tf.reduce_mean(logits2,axis=1),axis=1)
            # print(all_predict,"ggggggggggggggggggggggggggggggggggg")
            # print(tf.reduce_sum(all_predict,axis=0))
