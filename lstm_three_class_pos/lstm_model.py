...
class Model(object):
    ...
    """
    def __init__(self, config, batch, word_mat=None,pos_mat=None,char_mat=None,  filter_sizes=None, embedding_size=None,num_filters=None,trainable=True, l2_reg_lambda=0.0, keep_prob=0.9, graph=None):
        ...
        if trainable == True:
            self.input_x,self.c_pos, self.input_x1,self.q_pos,self.ch, self.qh, self.input_y, self.qa_id,self.alternatives_tokens = batch.get_next()  # self.y1 is (64, 3)self.alterh batch_size is[batch,3,alternative_len,chara_len]
        else:
            self.input_x,self.c_pos, self.input_x1,self.q_pos,self.ch, self.qh,self.alternatives_tokens= batch.get_next()  # self.y1 is (64, 3)self.alterh batch_size is[batch,3,alternative_len,chara_len]
        ...
        with tf.name_scope("embedding"):
            ...
            self.W = tf.get_variable("word_mat", initializer=tf.constant(word_mat, dtype=tf.float32),trainable=True)
            self.pos_W= tf.get_variable("pos_mat", initializer=tf.constant(word_mat, dtype=tf.float32),trainable=True)
            ...
            self.c_pos_em= tf.nn.embedding_lookup(self.pos_W, self.c_pos)
            self.embedded_chars1 = tf.nn.embedding_lookup(self.W, self.input_x1)
            self.q_pos_em=tf.nn.embedding_lookup(self.pos_W, self.q_pos)
            c_emb = tf.concat([self.embedded_chars, ch_emb,self.c_pos_em], axis=2)
            q_emb= tf.concat([self.embedded_chars1, qh_emb,self.q_pos_em], axis=2)
