...
class Model(object):
    def __init__(self, config, batch, word_mat=None, char_mat=None, trainable=True, opt=True, demo=False, graph=None):
        ...
        if trainable == True:
            self.c, self.q, self.ch, self.qh, self.alter, self.alterh, self.y1, self.qa_id = batch.get_next()  # self.y1 is (64, 3)self.alterh batch_size is[batch,3,alternative_len,chara_len]
        else:
            self.c, self.q, self.ch, self.qh, self.alter, self.alterh, self.qa_id = batch.get_next()  # self.y1 is (64, 3)self.alterh batch_size is[batch,3,alternative_len,chara_len]
        ...
        self.alter1_len = tf.reduce_sum(tf.cast(self.alter1_mask, tf.int32), axis=1)
        self.alter2_len = tf.reduce_sum(tf.cast(self.alter2_mask, tf.int32), axis=1)
        self.alter3_len = tf.reduce_sum(tf.cast(self.alter3_mask, tf.int32), axis=1)
        ...
        if opt:
            ...
            self.alter1= tf.slice(self.alter[:, 0, :], [0, 0], [N, self.aletr1_maxlen])
            self.alter2= tf.slice(self.alter[:, 1, :], [0, 0], [N, self.aletr2_maxlen])
            self.alter3= tf.slice(self.alter[:, 2, :], [0, 0], [N, self.aletr3_maxlen])



            self.c_mask = tf.slice(self.c_mask, [0, 0], [N, self.c_maxlen])
            self.q_mask = tf.slice(self.q_mask, [0, 0], [N, self.q_maxlen])
            self.alter1_mask = tf.slice(self.alter1_mask, [0, 0], [N, self.aletr1_maxlen])
            self.alter2_mask = tf.slice(self.alter2_mask, [0, 0], [N, self.aletr2_maxlen])
            self.alter3_mask = tf.slice(self.alter3_mask, [0, 0], [N, self.aletr3_maxlen])

            self.ch = tf.slice(self.ch, [0, 0, 0], [N, self.c_maxlen, CL])
            self.qh = tf.slice(self.qh, [0, 0, 0], [N, self.q_maxlen, CL])
            self.alter1h=tf.slice(self.alterh[:, 0, :, :],[0,0,0],[N,self.aletr1_maxlen,CL])
            self.alter2h=tf.slice(self.alterh[:, 1, :, :],[0,0,0],[N,self.aletr2_maxlen,CL])
            self.alter3h=tf.slice(self.alterh[:, 2, :, :],[0,0,0],[N,self.aletr3_maxlen,CL])
        ...
        self.alterh1_len = tf.reshape(tf.reduce_sum(tf.cast(tf.cast(self.alter1h, tf.bool), tf.int32), axis=2), [-1])
        self.alterh2_len = tf.reshape(tf.reduce_sum(tf.cast(tf.cast(self.alter2h, tf.bool), tf.int32), axis=2),[-1])
        self.alterh3_len = tf.reshape(tf.reduce_sum(tf.cast(tf.cast(self.alter3h, tf.bool), tf.int32), axis=2), [-1])
    ...
    def forward(self):
        ...
        with tf.variable_scope("Input_Embedding_Layer"):
            ...
            self.alternati_emb1 = tf.reshape(tf.nn.embedding_lookup(self.char_mat, self.alter1h),
                                             [N * AL1, CL, dc])  # (875, 25, 20)
            self.alternati_emb2 = tf.reshape(tf.nn.embedding_lookup(self.char_mat, self.alter2h),
                                             [N * AL2, CL, dc])  # (768, 16, 300)
            self.alternati_emb3 = tf.reshape(tf.nn.embedding_lookup(self.char_mat, self.alter3h),
                                             [N * AL3, CL, dc])  # (768, 16, 300)


            ch_emb = tf.nn.dropout(ch_emb, 1.0 - 0.5 * self.dropout)
            qh_emb = tf.nn.dropout(qh_emb, 1.0 - 0.5 * self.dropout)
            alternati_emb1 = tf.nn.dropout(self.alternati_emb1, 1.0 - 0.5 * self.dropout)
            alternati_emb2 = tf.nn.dropout(self.alternati_emb2, 1.0 - 0.5 * self.dropout)
            alternati_emb3 = tf.nn.dropout(self.alternati_emb3, 1.0 - 0.5 * self.dropout)
            ...
            alternati_emb1 = conv(alternati_emb1, d,bias=True, activation=tf.nn.relu, kernel_size=5, name="char_conv", reuse=True)
            alternati_emb2 = conv(alternati_emb2, d,bias=True, activation=tf.nn.relu, kernel_size=5, name="char_conv", reuse=True)
            alternati_emb3 = conv(alternati_emb3, d,bias=True, activation=tf.nn.relu, kernel_size=5, name="char_conv", reuse=True)
            ...
            alternati_emb1 = tf.reshape(alternati_emb1, [N, AL1, qh_emb.shape[-1]])
            alternati_emb2 = tf.reshape(alternati_emb2, [N, AL2, qh_emb.shape[-1]])
            alternati_emb3 = tf.reshape(alternati_emb3, [N, AL3, qh_emb.shape[-1]])


            c_emb = tf.nn.dropout(tf.nn.embedding_lookup(self.word_mat, self.c), 1.0 - self.dropout)
            q_emb = tf.nn.dropout(tf.nn.embedding_lookup(self.word_mat, self.q), 1.0 - self.dropout)
            alter_embedding1 = tf.nn.embedding_lookup(self.word_mat, self.alter1)  # 上下文
            alter_embedding2 = tf.nn.embedding_lookup(self.word_mat, self.alter2)  # 上下文
            alter_embedding3 = tf.nn.embedding_lookup(self.word_mat, self.alter3)  # 上下文

            c_emb = tf.concat([c_emb, ch_emb], axis=2) #把字符与对应的特征进行连接[batch,sequence_len,对应的输出维度]
            q_emb = tf.concat([q_emb, qh_emb], axis=2)
            alter_embedding1 = tf.concat([alter_embedding1, alternati_emb1], axis=2)
            alter_embedding2 = tf.concat([alter_embedding2, alternati_emb2], axis=2)
            alter_embedding3 = tf.concat([alter_embedding3, alternati_emb3], axis=2)
            ...
            alter_embedding1 = highway(alter_embedding1, size=d, scope="highway", dropout=self.dropout, reuse=True)
            alter_embedding2 = highway(alter_embedding2, size=d, scope="highway", dropout=self.dropout, reuse=True)
            alter_embedding3 = highway(alter_embedding3, size=d, scope="highway", dropout=self.dropout, reuse=True)
        ...
        with tf.variable_scope("Embedding_Encoder_Layer"):
            ...
            q = residual_block(q_emb,
                               ...
                               bias=False,
                               dropout=self.dropout)
            alter1 = residual_block(alter_embedding1,
                               num_blocks=1,
                               num_conv_layers=4,
                               kernel_size=7,
                               ...
                               bias=False,
                               dropout=self.dropout)
            alter2 = residual_block(alter_embedding2,
                                    num_blocks=1,
                                    num_conv_layers=4,
                                    kernel_size=7,
                                    ...
                                    bias=False,
                                    dropout=self.dropout)
            alter3 = residual_block(alter_embedding3,
                                    num_blocks=1,
                                    num_conv_layers=4,
                                    kernel_size=7,
        ...
        with tf.variable_scope('question_rnn'):
            ...
            output1,state1=tf.nn.dynamic_rnn(self.gru,alter1,initial_state=state)
            output2,state2=tf.nn.dynamic_rnn(self.gru,alter2,initial_state=state)
            output3,state3=tf.nn.dynamic_rnn(self.gru,alter3,initial_state=state)
        ...
        with tf.variable_scope("Output_Layer"):
            ...
            similary1=tf.expand_dims(self. cos_sine(weight_enc1,state1),axis=1)
            similary2=tf.expand_dims(self.cos_sine(weight_enc2,state2),axis=1)
            similary3=tf.expand_dims(self.cos_sine(weight_enc3,state3),axis=1)
            self.logits1=tf.nn.softmax(tf.concat([similary1,similary2,similary3],axis=1))
