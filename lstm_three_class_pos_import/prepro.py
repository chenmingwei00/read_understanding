...
def jieba_pos(sentence):
    words_p=pseg.cut(sentence.strip())
    words_pos=[]
    words=[]
    for w in words_p:
...
def process_file(filename, data_type,word_counter,pos_counter, char_counter,mode='train',size=None):
    """
    :param filename:
    ...
    with open(filename, "r") as fh:
        ...
        for line in tqdm(lines):
            try:
                ...
                context_tokens,context_pos = jieba_pos(context)#把段落对应的单词进行分词[one ,..., htrow]
                import_words_tokens=jieba_extract_word(context,tokkey=20)#提取前重要词汇20
                if len(context_tokens)>size:
                    context_tokens_sen=remove_unrelative(context,source["query"],size)
                    context_tokens,context_pos =jieba_pos(context_tokens_sen)
                    import_words_tokens=jieba_extract_word(context_tokens_sen)
                ...
                for pos in context_pos:#把对应的词性序列加入到字典中
                    pos_counter[pos]+=1
                ques = source["query"]
                ques_tokens,ques_pos = jieba_pos(ques)
                ques_chars = [list(token) for token in ques_tokens]
                ques_import_tokens=jieba_extract_word(ques,tokkey=5)
                ...
                new_alternative=[]
                alternatives_dict = {}
                for temp in alternatives:
                    alternatives_dict[temp] = len(temp)
                alternatives_dict = sorted(alternatives_dict.items(), key=lambda x: x[1])
                alternatives = [ele for ele, _ in alternatives_dict]
                flag = False
                if len(alternatives) == 3 and len(alternatives[0]) == len(alternatives[1]) :
                    if '不' in alternatives[0] or '没' in alternatives[0] or '否' in alternatives[0]:
                        new_alternative.append(alternatives[1])
                        new_alternative.append(alternatives[0])
                        new_alternative.append(alternatives[2])
                        flag = True
                    elif '不' in alternatives[1] or '没' in alternatives[1] or '否' in alternatives[1]:
                        new_alternative.append(alternatives[0])
                        new_alternative.append(alternatives[1])
                        new_alternative.append(alternatives[2])
                        flag = True
                else:
                    if len(alternatives) == 3 and '不' in alternatives[0] or '没' in alternatives[0] or '否' in alternatives[0]:
                        new_alternative.append(alternatives[1])
                        new_alternative.append(alternatives[0])
                        new_alternative.append(alternatives[2])
                        flag = True
                        ...
                        for temp_alter in alternatives:
                            if temp_alter.replace(min_word, '') == '不' or temp_alter.replace(min_word, '') == '没' or \
                                            '不' in temp_alter or '没' in temp_alter or '否' in temp_alter:
                ...
                if np.sum(ys1)==1:
                    ...
                    #     continue
                    # else:
                    example = {"context_tokens": context_tokens,"context_pos":context_pos,'import_words_tokens':import_words_tokens,"context_chars": context_chars,\
                               "ques_tokens": ques_tokens,'ques_pos':ques_pos,'ques_import_tokens':ques_import_tokens,"alternatives_tokens":np.array(new_alternative),
    ...
    return examples
def process_file_test(filename, data_type, word_counter, word_pos,char_counter,size=500):
    """
    :param filename:
    :param data_type: 我们先按照分类去做这件事情，类别分别是[肯定，否定，无法确定]三个类别去做
    :param word_counter: 产生单次级别的字典，频数对应段落单词对应多少个问答对以及加上该单词在问题中的出现频数，
    ...
    with open(filename, "r") as fh:
        ...
        for line in tqdm(lines):
            ...
            context = source["passage"]
            context_tokens,context_pos =jieba_pos(context)#把段落对应的单词进行分词[one ,..., htrow]
            import_words_tokens=jieba_extract_word(context)
            if len(context_tokens)>size:
                count+=1
                context_tokens_sen=remove_unrelative(context,source["query"],size)
                context_tokens,context_pos = jieba_pos(context_tokens_sen)
            ...
            if len(alternatives) == 3 and len(alternatives[0]) == len(alternatives[1]):
                ...
                if len(alternatives) == 3 and '不' in alternatives[0] or '没' in alternatives[0] or '否' in \
                        alternatives[0]:
                    try:
                        new_alternative.append(alternatives[1])
                        new_alternative.append(alternatives[0])
            ...
            for temp_alter in alternatives:
                if temp_alter not in new_alternative:
                    new_alternative.append(temp_alter)
            example = {"context_tokens": context_tokens,'context_pos':context_pos,'import_words_tokens':import_words_tokens, "context_chars": context_chars,\
                       "ques_tokens": ques_tokens,'ques_pos':ques_pos,'ques_import_tokens':ques_import_tokens,"ques_chars": ques_chars,
...
def get_embedding(counter, data_type, limit=-1, model=None,emb_file=None, emb_file1=None,size=None, vec_size=None, token2idx_dict=None):
    ...
    if len(emb_file)!=0 or len(emb_file1):
...
def build_features(config, examples, data_type, out_file, word2idx_dict, pos2idx_dict,char2idx_dict, is_test=False):
    ...
    for example in tqdm(examples):
        ...
        context_pos_idxs = np.zeros([para_limit], dtype=np.int32) #对应的paragraph的单词下标初始化，最大为para_limit
        context_import_idxs = np.zeros([20], dtype=np.int32) #对应的paragraph的单词下标初始化，最大为para_limit
        ...
        ques_pos_idxs = np.zeros([ques_limit], dtype=np.int32)#], dtype=np.int32)#每一个对应段落的字符初始化
        ques_import_idxs= np.zeros([5], dtype=np.int32)
        ...
        for i, import_token in enumerate(example['import_words_tokens']):
            if i < 20:
                context_import_idxs[i] = _get_word(import_token)
        ...
        for i, important_token in enumerate(example['ques_import_tokens']):
            if i < 5:
                ques_import_idxs[i] = _get_word(important_token)
        ...
        if is_test:
            ...
            record = tf.train.Example(features=tf.train.Features(feature={
                ...
                "context_import_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[context_import_idxs.tostring()])),
                "ques_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[ques_idxs.tostring()])),
                "ques_pos_idxs":tf.train.Feature(bytes_list=tf.train.BytesList(value=[ques_pos_idxs.tostring()])),
                "ques_import_idxs": tf.train.Feature(bytes_list=tf.train.BytesList(value=[ques_import_idxs.tostring()])),
...
def prepro(config):
    ...
    word_counter,pos_counter ,char_counter = Counter(), Counter(),Counter()
    ...
    train_examples = process_file(config.train_file, "train", word_counter,pos_counter ,char_counter,mode='train',size=config.para_limit)
    print(train_examples[0])
    dev_examples= process_file(config.dev_file, "dev", word_counter,pos_counter,char_counter,mode='dev',size=config.para_limit) #同样的处理
    print(dev_examples[0])
    test_examples = process_file_test(config.test_file, "test", word_counter, pos_counter,char_counter,size=config.para_limit)
    ...
    pos2idx_dict = None  # 获取字符对应的embedding以及对应的字符对应下标
    if os.path.isfile(config.pos2idx_file):
        with open(config.pos2idx_file, "r") as fh:
            pos2idx_dict = json.load(fh)
    pos_emb_mat, pos2idx_dict = get_embedding(pos_counter, "word", emb_file={}, emb_file1={},
                                                size=config.glove_word_size, vec_size=config.pos_dim,
                                                token2idx_dict=pos2idx_dict)
    ...
    build_features(config, train_examples, "train",
                   config.train_record_file, word2idx_dict, pos2idx_dict,char2idx_dict)
    dev_meta = build_features(config, dev_examples, "dev",
                              config.dev_record_file, word2idx_dict, pos2idx_dict,char2idx_dict)
    test_meta = build_features(config, test_examples, "test",
                               config.test_record_file, word2idx_dict,pos2idx_dict, char2idx_dict, is_test=True)
    print(test_meta)
    print('finsh_oit')
    # # save(config.word_emb_file, word_emb_mat, message="word embedding")
    pickle.dump(word_emb_mat,open(config.word_emb_file,'wb'), pickle.HIGHEST_PROTOCOL)
    pickle.dump(pos_emb_mat,open(config.pos_emb_file,'wb'),pickle.HIGHEST_PROTOCOL)
    ...
    save(config.pos2idx_file,pos2idx_dict,message="pos2idx")
