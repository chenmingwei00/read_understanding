...
class Model(object):
    ...
    """
    def __init__(self, config, batch, word_mat=None,char_mat=None,pos_mat=None,filter_sizes=None, embedding_size=None,num_filters=None,trainable=True, l2_reg_lambda=0.0, keep_prob=0.9, graph=None):
        ...
        if trainable == True:
            self.input_x,self.c_pos,self.c_important, self.input_x1,self.q_pos,self.q_important,self.ch, self.qh, self.input_y, self.qa_id,self.alternatives_tokens = batch.get_next()  # self.y1 is (64, 3)self.alterh batch_size is[batch,3,alternative_len,chara_len]
        else:
            self.input_x,self.c_pos,self.c_important, self.input_x1,self.q_pos,self.q_important,self.ch, self.qh,self.alternatives_tokens= batch.get_next()  # self.y1 is (64, 3)self.alterh batch_size is[batch,3,alternative_len,chara_len]
        ...
        self.c_pos_mask = tf.cast(self.c_pos, tf.bool)  # 这里是判断出每一个数据集的context对应实际句子长度的位置(64,400)
        ...
        self.c_pos_len = tf.reduce_sum(tf.cast(self.c_pos_mask, tf.int32), axis=1)  # 每一个训练数据集实际长度
        self.q_pos_len = tf.reduce_sum(tf.cast(self.q_pos_mask, tf.int32), axis=1)  # 每一个问题的实际长度
        self.c_important_len =tf.reduce_sum(tf.cast(self.c_important_mask,tf.int32),axis=1)
        self.q_important_len =tf.reduce_sum(tf.cast(self.q_important_mask,tf.int32),axis=1)
        ...
        self.words_embedding = tf.get_variable("word_mat", initializer=tf.constant(word_mat, dtype=tf.float32), trainable=True)
        self.pos_W_embedding = tf.get_variable("pos_mat", initializer=tf.constant(pos_mat, dtype=tf.float32), trainable=True)
        ...
        with tf.variable_scope("Input_Embedding_Layer"):#字符表示方法
            ...
            _, (state_fw, state_bw) = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, ch_emb, self.ch_len,dtype=tf.float32)  # self.ch_len表示训练数据集所有字符平摊之后，实际字符的长度,sequence_length=[bacth_size] is N * PL, because
        ...
        with tf.variable_scope("Iportant_Embedding_Layer"):
            c_important_emb=tf.nn.embedding_lookup(self.words_embedding, self.c_important)
            q_important_emb=tf.nn.embedding_lookup(self.words_embedding, self.q_important)
            c_important_emb = tf.nn.dropout(c_important_emb, 1.0 - 0.5 * self.dropout)
            q_important_emb = tf.nn.dropout(q_important_emb, 1.0 - 0.5 * self.dropout)
            cell_fw = tf.contrib.rnn.GRUCell(dg)  # 按照字符有多少个gru神经单元
            cell_bw = tf.contrib.rnn.GRUCell(dg)
            _, (state_fw, state_bw)= tf.nn.bidirectional_dynamic_rnn(
                cell_fw, cell_bw, c_important_emb, self.c_important_len,
                dtype=tf.float32)  # self.ch_len表示训练数据集所有字符平摊之后，实际字符的长度,sequence_length=[bacth_size] is N * PL, because
            # char_hidden is 100 so state_fw and state_bw is [N * PL,100]
            c_important_emb = tf.concat([state_fw, state_bw], axis=1)  # [N * PL,200]
            _,(state_fw, state_bw)= tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, q_important_emb, self.q_important_len,
                                                                      dtype=tf.float32)  # state_* [N*QL]
            q_important_emb = tf.concat([state_fw, state_bw], axis=1)  # question_emd is [,200]
        ...
        with tf.variable_scope("pos_Embedding_Layer"):
            ...
            (state_fw, state_bw), _ = tf.nn.bidirectional_dynamic_rnn(
                cell_fw, cell_bw, c_pos_em, self.c_pos_len,
                dtype=tf.float32)  # self.ch_len表示训练数据集所有字符平摊之后，实际字符的长度,sequence_length=[bacth_size] is N * PL, because
            # char_hidden is 100 so state_fw and state_bw is [N * PL,100]
            c_pos_em = tf.concat([state_fw, state_bw], axis=2)  # [N * PL,200]
            (state_fw, state_bw), _ = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, q_pos_em,
                                                                      self.q_pos_len,
                                                                      dtype=tf.float32)  # state_* [N*QL]
            q_pos_em = tf.concat([state_fw, state_bw], axis=2)  # question_emd is [,200]
        ...
        with tf.name_scope("embedding"):
            ...
            self.embedded_chars = tf.nn.embedding_lookup(self.words_embedding, self.input_x)
            self.embedded_chars1 = tf.nn.embedding_lookup(self.words_embedding, self.input_x1)
