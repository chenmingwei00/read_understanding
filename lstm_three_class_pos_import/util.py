...
def get_record_parser(config, is_test=False):
    def parse(example):
        ...
        if is_test!=True:
            features = tf.parse_single_example(example,
                                               features={
                                                   ...
                                                   "context_import_idxs": tf.FixedLenFeature([], tf.string),
                                                   "ques_idxs": tf.FixedLenFeature([], tf.string),
                                                   "ques_pos_idxs":tf.FixedLenFeature([], tf.string),
                                                   "ques_import_idxs": tf.FixedLenFeature([], tf.string),
                                                   "context_char_idxs": tf.FixedLenFeature([], tf.string),
                                                   "ques_char_idxs": tf.FixedLenFeature([], tf.string),
                                                   "alternatives_tokens":tf.FixedLenFeature([], tf.string),
                                                   "y1": tf.FixedLenFeature([], tf.string),
        ...
        context_idxs = tf.reshape(tf.decode_raw(features["context_idxs"], tf.int32), [para_limit])
        context_pos_idxs = tf.reshape(tf.decode_raw(features["context_pos_idxs"], tf.int32), [para_limit])
        context_import_idxs = tf.reshape(tf.decode_raw(features["context_import_idxs"], tf.int32), [20])
        ques_idxs = tf.reshape(tf.decode_raw(features["ques_idxs"], tf.int32), [ques_limit])
        ques_pos_idxs = tf.reshape(tf.decode_raw(features["ques_pos_idxs"], tf.int32), [ques_limit])
        ques_import_idxs = tf.reshape(tf.decode_raw(features["ques_import_idxs"], tf.int32), [5])
        context_char_idxs = tf.reshape(tf.decode_raw(features["context_char_idxs"], tf.int32), [para_limit, char_limit])
        ques_char_idxs = tf.reshape(tf.decode_raw(features["ques_char_idxs"], tf.int32), [ques_limit, char_limit])
        ...
        if is_test!=True:
            ...
            return context_idxs,context_pos_idxs,context_import_idxs,ques_idxs,ques_pos_idxs,ques_import_idxs,context_char_idxs,\
                   ques_char_idxs,y1,qa_id,alternatives_tokens
        else:
            qa_id = features["id"]
            return context_idxs,context_pos_idxs,context_import_idxs,ques_idxs,ques_pos_idxs,\
                   ques_import_idxs,context_char_idxs,\
                   ques_char_idxs,alternatives_tokens,qa_id
    return parse
