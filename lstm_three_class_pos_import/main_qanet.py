...
def train(config):
    ...
    with open(config.pos_emb_file, "rb") as fh:
        pos_mat = np.array(pickle.load(fh), dtype=np.float32)#加载字符向量对应的
    ...
    model = Model(config, iterator, word_mat=word_mat,char_mat=char_mat,pos_mat=pos_mat,filter_sizes=[3,4,5], embedding_size=300,num_filters=128)
            ...
            try:
                ...
                if global_step % config.checkpoint == 0:
                    ...
                    if dev_loss > loss_save:
                        ...
                        figure_path=os.path.join(config.save_dir,'im1.png')
