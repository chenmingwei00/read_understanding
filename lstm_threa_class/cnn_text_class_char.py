...
class Model(object):
    ...
    def __init__(self, config, batch, word_mat=None,char_mat=None,  filter_sizes=None, embedding_size=None,num_filters=None,trainable=True, l2_reg_lambda=0.0, keep_prob=0.9, graph=None):
        ...
        # Embedding layer
        N, PL, QL, CL, d, dc= config.batch_size,config.para_limit,config.ques_limit,config.char_limit, config.hidden, config.char_dim
        with tf.variable_scope("Input_Embedding_Layer"):
            ...
            ch_emb = tf.reduce_max(ch_emb, axis=1)  # 求出横向唯独的最大特征，这里可以用k_max尝试
        ...
        with tf.device('/gpu:0'), tf.name_scope("embedding"):
            ...
            self.embedded_chars_expanded = tf.concat([self.embedded_chars, ch_emb], axis=2)
            self.embedded_chars_expanded1= tf.concat([self.embedded_chars1, qh_emb], axis=2)
