...
def residual_block(inputs, num_blocks, num_conv_layers, kernel_size, mask = None,
    ...
    with tf.variable_scope(scope, reuse = reuse):
        ...
        outputs = inputs
        sublayer = 1
        total_sublayers = (num_conv_layers + 2) * num_blocks  #6
        for i in range(num_blocks):
            outputs = add_timing_signal_1d(outputs)#增加一个随机信号，这是把时间序列特性加上去了，周期特性
...
def conv_block(inputs, num_conv_layers, kernel_size, num_filters,
    ...
    with tf.variable_scope(scope, reuse = reuse):
        outputs = tf.expand_dims(inputs,2)#[batch,quequce_len,1,75]
        l, L = sublayers
        for i in range(num_conv_layers):
            residual = outputs
            outputs = norm_fn(outputs, scope = "layer_norm_%d"%i, reuse = reuse)#于是正则化就是用输出减去均值，然后再用
            if (i) % 2 == 0:
...
def depthwise_separable_convolution(inputs, kernel_size, num_filters,
    ...
    with tf.variable_scope(scope, reuse = reuse):
        shapes = inputs.shape.as_list() #[batch,squen_len,1,75]
...
def add_timing_signal_1d(x, min_timescale=1.0, max_timescale=1.0e4):
    """Adds a bunch of sinusoids of different frequencies to a Tensor.            对tensor增加不同频率的正弦波
      用来增加时序信息
    Each channel of the input Tensor is incremented by a sinusoid of a different  对于不同的channel增加一个正弦函数
    frequency and phase.
    This allows attention to learn to use absolute and relative positions.       这允许attention学习相对或者绝对位置的信息学
    Timing signals should be added to some precursors of both the query and the  时序信号应该增加到问题或者记录的attentioon
...
def optimized_trilinear_for_attention(args, c_maxlen, q_maxlen, input_keep_prob=1.0,
    ...
    arg_size = arg0_shape[2] #每一个词向量的表示维度
    dtype = args[0].dtype
    ...
    with tf.variable_scope(scope):
        ...
        subres0 = tf.tile(dot(droped_args[0], weights4arg0), [1, 1, q_maxlen])#利用对于权重求得每一个单词的综合表示一个数
